...
class CookieStoreManager {
	constructor() {
		...
		function cookieRequestProxy(options) {
			...
			if (api.platform !== 'h5' && options.cookie) {
				...
				function handleCookies(response) {
					...
					// 获取响应 cookies
					let responseCookies = response.header ? response.header['Set-Cookie'] || response.header[
						'set-cookie'] : ''
					if (responseCookies) {
						// 处理QQ小程序下cookie分隔符问题：https://github.com/charleslo1/weapp-cookie/issues/39
						responseCookies = responseCookies.replace(
							/;((?!Expires|Max-Age|Domain|Path|SameSite)[^\s;]*?)=/gi, ',$1')
						...
						if (typeof uni !== "undefined") responseCookies = responseCookies.substring(1,
							responseCookies.length - 1);
		...
		// 返回 cookieStore
		this.cookieStoreInstance = cookieStore
	}

	getCookieStoreInstace() {
		return this.cookieStoreInstance
	}

	static getInstance() {
		console.log(`%c CookieStoreManager  CookieStoreManager.instance`, 'color:pink', this.instance)
		if (!this.instance) {
			this.instance = new CookieStoreManager()
		}
		return this.instance
	}
}

const cookieStore = CookieStoreManager.getInstance().getCookieStoreInstace()
